...
namespace SupremeMelee
{
    ...
    public static class SupremeVerbUtility
	{
		...
		public static bool IsDualWielding(this Pawn pawn)
		{
			...
			{
				result = equipment.AllEquipmentListForReading.Count((ThingWithComps thing) => thing.def.IsMeleeWeapon && thing.def.equipmentType == EquipmentType.Primary) >= 2;
			}
			else
			{
				result = false;
		...
		public static bool IsWieldingMeleeWeapons(this Pawn pawn)
		{
			...
			if ((equipment = pawn.equipment) != null)
			{
				result = equipment.AllEquipmentListForReading.Any((ThingWithComps thing) => thing.def.IsMeleeWeapon && thing.def.equipmentType == EquipmentType.Primary) || pawn.health.hediffSet.GetHediffsVerbs() != null;
			}
			else
			{
				result = false;
